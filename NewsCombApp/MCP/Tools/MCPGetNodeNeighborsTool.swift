...
enum MCPGetNodeNeighborsTool {
    static func run(arguments: [String: Value], database: any MCPDatabaseReader = Database.shared) throws -> String {
        ...
        let (node, edges) = try database.read { db -> (Row?, [Row]) in
            ...
            guard let nodeRow else { return (nil, []) }
            let nodeId: Int64 = nodeRow["id"]

            let edgeRows = try Row.fetchAll(db, sql: """
                SELECT DISTINCT
                    he.id AS edge_id, he.label AS edge_label,
                    GROUP_CONCAT(CASE WHEN hi2.role = 'source' THEN hn2.label END, ', ') AS source_nodes,
                    GROUP_CONCAT(CASE WHEN hi2.role = 'target' THEN hn2.label END, ', ') AS target_nodes,
        ...
        guard let node else {
            throw MCPToolError.notFound("No concept found matching '\(nodeLabel)'. Try search_concepts to find the exact label.")
        }
        ...
        if let df, df > 0 { output += " — \(df) events" }
        output += "\n\n"

        guard !edges.isEmpty else {
            return output + "No relationships found for this concept.\n"
        }
        ...
        for row in edges {
            ...
            output += "- \(sources ?? "?") **\(edgeLabel)** \(targets ?? "?")\n"
            if let provenance, !provenance.isEmpty {
                let truncated = provenance.count > 200 ? String(provenance.prefix(200)) + "..." : provenance
                output += "  > \(truncated)\n"
            }
        }
        return output
