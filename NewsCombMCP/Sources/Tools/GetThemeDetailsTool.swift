...
enum GetThemeDetailsTool {
    static func run(arguments: [String: Value], database: ReadOnlyDatabase) throws -> String {
        guard let clusterId = arguments["cluster_id"]?.intValue else {
            throw ToolError.missingParameter("cluster_id")
        }

        let result = try database.read { db -> String in
            // Fetch the cluster
            guard let cluster = try Row.fetchOne(db, sql: """
                SELECT cluster_id, build_id, label, size, summary,
                       top_entities_json, top_rel_families_json
                FROM clusters
                WHERE cluster_id = ?
            """, arguments: [clusterId]) else {
                throw ToolError.notFound("Theme cluster \(clusterId) not found. Use get_themes to list available clusters.")
            }
            ...
            if let json = topEntitiesJson,
               ...
               let entities = try? JSONDecoder().decode([RankedEntity].self, from: data),
               !entities.isEmpty {
                output += "### Top Entities\n"
                for entity in entities.prefix(10) {
                    output += "- \(entity.label) (score: \(String(format: "%.2f", entity.score)))\n"
                }
                output += "\n"
            }
            ...
            if let json = topRelFamiliesJson,
               let data = json.data(using: .utf8),
               let families = try? JSONDecoder().decode([RankedFamily].self, from: data),
               !families.isEmpty {
            ...
            // Exemplar events (edges closest to cluster centroid)
            let exemplars = try Row.fetchAll(db, sql: """
                SELECT ce.rank, he.label AS edge_label,
                       GROUP_CONCAT(
                           CASE WHEN hi.role = 'source' THEN hn.label END, ', '
                       ) AS sources,
                       GROUP_CONCAT(
                           CASE WHEN hi.role = 'target' THEN hn.label END, ', '
                       ) AS targets
            ...
            // Source articles for this cluster's events
...
private struct RankedEntity: Codable {
    let label: String
    let score: Double
}

private struct RankedFamily: Codable {
    let family: String
    let count: Int
}
