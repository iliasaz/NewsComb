...
This project uses [GRDB.swift](https://github.com/groue/GRDB.swift) for SQLite persistence. Do not use SwiftData.

### Schema definition

- Define database models as structs conforming to `FetchableRecord`, `PersistableRecord`, and `Codable`.
- Use `static let databaseTableName` to specify the table name.
- Define column mappings with an enum conforming to `ColumnExpression`.
- Design schemas with proper SQLite knowledge: indexes, foreign keys, and normalization.
...
- Run tests with: `xcodebuild test -scheme NewsCombApp -destination 'platform=macOS,arch=arm64' -skip-testing:NewsCombAppUITests`
  - All targets (host app, NewsCombAppTests, NewsCombAppUITests) share team `6CGNH3LTV7`, so default signing works without any `CODE_SIGNING_ALLOWED=NO` workaround.
  - `-skip-testing:NewsCombAppUITests` is recommended for headless command-line runs since UI tests require an interactive window-server session.
